//! OKX Public API Example
//!
//! This example demonstrates fetching public market data from OKX using the unified CCXT API.
//!
//! Usage:
...
async fn main() -> Result<()> {
    println!("=== CCXT-RS OKX Public API Test ===\n");

    // Create OKX exchange instance
    let okx = ccxt::okx::Okx::builder()
        .use_aws(false)  // Use main endpoint (or set true for AWS)
        .build()?;

    println!("Exchange: {} ({})", okx.name(), okx.id());
    println!("Type: {:?}\n", okx.exchange_type());
    ...
    let orderbook = okx.fetch_order_book("BTC/USDT", Some(5)).await?;
    ...
    let trades = okx.fetch_trades("BTC/USDT", None, Some(5)).await?;
    ...
    // =========================================================================
    // 7. Fetch Exchange Status
    // =========================================================================
    println!("7. Fetching exchange status...");
    let status = okx.fetch_status().await?;
    println!("   Status: {}", status.status);
    println!("   ✓ Success\n");

    // =========================================================================
    // 8. Compare Different Timeframes
    // =========================================================================
    println!("8. Comparing different timeframes (last candle)...");
